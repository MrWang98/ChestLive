...
for a in range(5):
    for b in range(5):
        ...
        temp_image = np.stack((temp_image[:, :, 0],) * 3, axis=2)  # Increase one dimension.
        temp_image *= 255
        temp_image = np.clip(temp_image, 0, 255).astype("uint8")  # limit the range to [0,255]
        if b == 2:
            axarr[a, b].set_title("Class : " + sample_keys[a])
        axarr[a, b].imshow(temp_image, cmap="gray")  # Show picture.
...
for i, ax in zip(range(5), axarr):#将对应元素打包成一个元组。
