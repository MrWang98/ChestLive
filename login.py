...
import ReadData_login as ReadData
        ...
        for class_idx, class_obj in enumerate(label_subset):
            ...
            if split:
                ...
                if test:
                  ...
                  # print("{},l:{}".format(label_subset[class_idx], len(self.data[label_subset[class_idx]])))
                  ...
                  # testImage[class_obj] = random.choices(self.data[label_subset[class_idx]][shots+1:],k=test_size)
                  testImage[class_obj]=self.data[label_subset[class_idx]][shots+1:shots+test_size+1]
                  testLabels[class_obj][0:] = class_idx
        ...
        if test:
          ...
          for i in range(l):
            for k in label_subset:
              ...
              # print("length:{},idx:{},key:{}".format(len(list(testImage[k])),i,k))
              # print()
...
for i in range(number):
  ...
  for l in keys:
    ...
    if l not in all_count:
      all_count[l]=0
    if l not in person:
        person[l]=[0 for n in range(test_size)]
        test_count[l]=[0 for n in range(test_size)]
  ...
  for images, labels in test_set:
      ...
      if (person[keys[0]][count]==0)&(int(labels[0])!=preds[0]):
          test_count[keys[0]][count]=test_count[keys[0]][count]+1
      elif (person[keys[0]][count]==0)&(int(labels[0])==preds[0]):
          person[keys[0]][count]=1

      if (person[keys[1]][count]==0)&(int(labels[1])!=preds[1]):
          test_count[keys[1]][count]=test_count[keys[1]][count]+1
      elif (person[keys[1]][count]==0)&(int(labels[1])==preds[1]):
          person[keys[1]][count]=1
...
# with open("npy/r_{}.txt".format(person_number),"w") as f:
#     f.write(str(result))
with open("npy/CDF2.txt","w") as f:
