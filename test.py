...
class testDataset:
    ...
    def get_mini_dataset(
        self, batch_size, repetitions, shots, num_classes, name, situation, split=False, test=False
    ):
        testImage = {}
        testLabels = {}
        ...
        for class_idx, class_obj in enumerate(label_subset):
            ...
            if split:
                ...
                if test:
                  ...
                  if(label_subset[class_idx]==name):
                    print(label_subset[class_idx]+"_"+situation)
                    testImage[class_obj] = random.choices(self.test_data[label_subset[class_idx]+"_"+situation],k=test_size)
...
for dir in dir_list:
    ...
    if(len(t)>2):
        ...
        t2=t[1]
    files.append([t1,t2])
with open(r"Set/result_{}.txt".format(files[0][0]),"a") as f:
    f.write("\n")
for file in files:
  number = 1000
  acc = {}
  ...
  for i in range(number):
    ...
    train_set, test_images, test_labels, keys, test_set = testSet.get_mini_dataset(
          eval_batch_size, eval_iters, shots, classes, name=file[0], situation=file[1], split=True, test=True
      )
    # print(test_set)
    for l in keys:
  ...
  with open(r"Set/result_{}.txt".format(file[0]),"a") as f:
