...
def LoadData(training):
    finalDir = "/home/xuemeng/Proj/ChestMotion/Final" #Final文件夹的路径，即所有收集以人名命名的文件夹所在的目录的路径
    scaleOfTrainSet = 2 / 3   #可在这里调整训练集占总数据的比例
    return loadAllData(finalDir,scaleOfTrainSet,training)  #第二个参数默认值是True，意思是取训练集

if __name__ == '__main__':
    finalDir = "/home/xuemeng/Proj/ChestMotion/Final"
    scaleOfTrainSet = 1
    allData, cupLabels = loadAllData(finalDir, scaleOfTrainSet, True)
    print("\nimageData:\t"+str(allData.shape))
    print("labels:\t\t"+str(cupLabels.shape)+"\n")
