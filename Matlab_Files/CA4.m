...
% Numbers(1, 2, 3...) inside the dir represent the epoch time.(Vowel_end Start from 0.06, increased by 0.02 each epoch.) 
...
%% Find the proper vowel length(0.06, 0.08, 0.1 ...)
% for epoch = 1:3
    epoch = 1;
    % str = sprintf('%s%d%s', 'epoch ', epoch, ' Start ...');
    % disp(str)
    times_number = 1;
    % vowel_end = 0.06 + (epoch-1)*0.02; % set vowel length
    vowel_end = 0.1;
    file_path = sprintf('%s%s%s%s%d%s%s%s', result_root_dir, name, '/', 'problem_nums-', vowel_end, '-', name, '.txt'); % set record file

    while times_number <= sample_num
        try
            % Load wav file.
            path_voice2 = data_dir;
            path_voice3 = '.wav';
