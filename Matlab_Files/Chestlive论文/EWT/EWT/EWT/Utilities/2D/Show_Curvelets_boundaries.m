...
elseif option==3
    a=Bw{1}*floor(size(f,2)/(2*pi));
    b=Bw{1}*floor(size(f,1)/(2*pi));
    drawEllipse(floor(size(f,2)/2)+1,floor(size(f,1)/2)+1,a,b,color);
    ...
    for t=1:length(Bt)-1
        for s=1:length(Bw{t+1})
            a=Bw{t+1}(s)*floor(size(f,2)/(2*pi));
            b=Bw{t+1}(s)*floor(size(f,1)/(2*pi));
            drawArcEllipse(floor(size(f,2)/2)+1,floor(size(f,1)/2)+1,a,b,Bt(t),Bt(t+1),color);
        end
    end
    
    %last angular sector
    for s=1:length(Bw{end})
        a=Bw{end}(s)*floor(size(f,2)/(2*pi));
        b=Bw{end}(s)*floor(size(f,1)/(2*pi));
        drawArcEllipse(floor(size(f,2)/2)+1,floor(size(f,1)/2)+1,a,b,Bt(end),Bt(1)+pi,color);
    end
end
