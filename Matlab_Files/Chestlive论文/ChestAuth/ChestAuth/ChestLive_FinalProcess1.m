...
%     figure(10);plot(ETic,Amp(:, 3715));title(['Bins:', num2str(3715)]);xlabel('time (s)'); %1858 
    ...
    j = 1; 
    for i=3705:1:3726 %3705:1:3725 1850:1:1865
        bin_Y = fft(Amp(:, i));
        [bin_x, bin_yt] = max( abs(bin_Y(5:end, 1)) );
        bin_y = (4+bin_yt -1)*EFs / Amp_r;
        if( bin_y > 0.16 && bin_y < 0.6 )
            SubEsd(j,:) =Amp(:, i);
            j = j+1;
        end
    end
...
%     [mappedD]= compute_mapping(mappedD, 'KPCA',no_dims); 
    ...
    figure(12);plot(ETic, MappedData_KPCA);title('KPCA ');xlabel('time (s)');
...
%         Esd = smooth(Esd,10);
        ...
        figure(13);plot(ETic , Esd );title("Denoise Breathing");
    end
...
% Store Data
if (handle_arr(10) == 1)
    path_store0  = '/Users/mengxue/Documents/Paper/ChestAuthentication/Material/BigData/Final/ZhuXiaoTian/Matlab/HeySiri';
    path_store2 = '.mat';
