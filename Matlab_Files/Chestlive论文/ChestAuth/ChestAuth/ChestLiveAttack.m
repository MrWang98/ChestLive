...
close all;
clear ;clc;
%% Auguments setting VoiceStoreOuRunMin1 VoiceStoreQinDang1 VoiceStoreXueMeng1
times_number = 20; % 采集数据的编号 0-100
vowel_end = 0.05; % 有话语音段选择参数：调整范围，0.05 - 0.4。根据图106调整。
deleteseg_num = 0; % 删除多余的语言段：可选项 0 1 2 3 
Data_number = 80 + times_number; % 当存储数据编号，默认是采集数据的编号。

VictimsNumber = 3; % 1-OuRunMin 2-QinDang 3-XueMeng
AttackerNumber = 5; %ZhuXiaoTian KuangruiLin GQY LPY LiLinWei
handle_esd = [
    1 % 分帧 求时间 1 
...
if(VictimsNumber == 1)
    Victims = 'OuRunMin';
    voiceSegment = load('/Users/mengxue/Documents/Paper/ChestAuthentication/Material/BigData/Final/Attack/Matlab/VoiceStoreOuRunMin1.mat'); 
end
if(VictimsNumber == 2)
    Victims = 'QinDang';
    voiceSegment = load('/Users/mengxue/Documents/Paper/ChestAuthentication/Material/BigData/Final/Attack/Matlab/VoiceStoreQinDang1.mat'); 
end
if(VictimsNumber == 3)
    Victims = 'XueMeng';
    voiceSegment = load('/Users/mengxue/Documents/Paper/ChestAuthentication/Material/BigData/Final/Attack/Matlab/VoiceStoreXueMeng1.mat'); 
end

if(AttackerNumber ==1)
    Attacker = 'ZhuXiaoTian';
end
if(AttackerNumber ==2)
    Attacker = 'KuangRuiLin';
end
if(AttackerNumber ==3)
    Attacker = 'GQY';
end
...
path_voice2 = sprintf('%s%s%s','/Users/mengxue/Documents/Paper/ChestAuthentication/Material/BigData/Final/', Attacker, '/CollectedData/WAV文件/getWav');
path_undulate2 = sprintf('%s%s%s','/Users/mengxue/Documents/Paper/ChestAuthentication/Material/BigData/Final/', Attacker, '/CollectedData/BINS文件/TEST_BINS');
path_store0 = sprintf('%s%s%s','/Users/mengxue/Documents/Paper/ChestAuthentication/Material/BigData/Final/Attack/Matlab/', Victims, '/HeySiri');
...
if (handle_arr(8) == 1)
    ...
    voiceCundualte1 = xcorr(voiceSegment.voice_store1, Result_undulate{1,1},'none');
    voiceCundualte2 = xcorr(voiceSegment.voice_store2, Result_undulate{1,2},'none');
...
% 计算 crosscor
if (handle_arr(9) == 1)
    [acor1,lag1] = crosscorr(voiceSegment.voice_store1,Result_undulate{1,1});
    [acor2,lag2] = crosscorr(voiceSegment.voice_store2,Result_undulate{1,2});
    [~,I1] = max(abs(acor1));
    [~,I2] = max(abs(acor2));
    lagDiff1 = lag1(I1);
