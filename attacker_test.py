...
attacker={}
filelist=os.listdir('split2/test')
for p in filelist:
    s=p.split("_")
    if len(s)>1:
        attacker[s[0]]=[]
...
for i in range(number):
  ...
  for images, labels in test_set:
      ...
      if(keys[0].split("_")[0]==keys[1].split("_")[0]):
          attacker[keys[0]].append([[0,labels[0]],preds[0]])
          attacker[keys[1]].append([[0,labels[1]],preds[1]])
...
with open("npy/result_{}.txt".format(person_number),"w") as f:
    f.write(str(person))
with open("npy/attacker_result.txt","a") as f:
